"""
Repopulate the S3 bucket with newly scraped and processed jobs.

Usage (PowerShell):
  python scripts/refresh_s3.py --search-terms "Data Scientist,Machine learning engineer,AI engineer,Data Analyst" --locations "United States,Remote" --results-per-site 400 --s3-prefix "" --timestamp
...
Notes:
- Optimized for trend analysis: uses smaller samples (400 results/site) for faster execution while maintaining statistical significance
...
# Ensure project root is on sys.path when running as a script (python scripts/refresh_s3.py)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Repopulate S3 with scraped/processed job data")
    ...
    p.add_argument("--results-per-site", type=int, default=int(os.getenv("RESULTS_PER_SITE", "400")), help="Max results per site (default: 400 for faster trend analysis)")
...
def main() -> int:
    ...
    # Scrape fresh data
    print(f"Scraping with terms={search_terms}, locations={locations}, results/site={results_per_site}")
    scraper = JobScraper(search_terms=search_terms, locations=locations, results_per_site=results_per_site)
